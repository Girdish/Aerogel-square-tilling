...
static Ref_t createDetector(Detector& desc, xml::Handle_t handle, SensitiveDetector sens) {
  ...
  auto airgapThickness = airgapElem.attr<double>(_Unicode(thickness));
    // Rim and Rib 
  auto rimWidth = desc.constant<double>("DRICH_rim_width");
  auto nTilesX = desc.constant<int>("DRICH_rib_tiles_x");
  auto nTilesY = desc.constant<int>("DRICH_rib_tiles_y");
  auto ribThickness = desc.constant<double>("DRICH_rib_thickness");
  auto ribMat = desc.material(desc.constant<std::string>("DRICH_rib_material"));
...
// Add airgap and filter solids 
Cone airgapSolid(airgapThickness / 2, 
                 radiatorRmin + boreDelta * aerogelThickness / vesselLength,
                 radiatorRmax + snoutDelta * aerogelThickness / snoutLength,
...
float outerRmaxBack = radiatorRmax + boreDelta * aerogelThickness / snoutLength;

// Define a conical shell for outer rim to match the aerogel cone
Cone rimOuterSolid(aerogelThickness / 2,
                   radiatorRmax,
                   radiatorRmax + rimWidth,
                   outerRmaxBack,
                   outerRmaxBack + rimWidth);

// aerogel placement 
auto radiatorPos = Position(0., 0., radiatorFrontplane + 0.5 * aerogelThickness) + originFront;
auto aerogelPlacement = Translation3D(radiatorPos) * RotationY(radiatorPitch);

auto rimTransform = Transform3D(Translation3D(radiatorPos) * RotationY(radiatorPitch));

// rib structure
float originalSideLength = 2 * (radiatorRmax);
...
// Creating a union of all ribs and rims
...
ribUnionSolid = UnionSolid(ribUnionSolid, rimOuterSolid, rimTransform);

// Intersect-subtract operations 
...
// Place airgap and filter 
if (!debugOptics) {
    // Airgap placement (after aerogel)
    auto airgapPlacement = Translation3D(radiatorPos) * 
                          RotationY(radiatorPitch) *
                          Translation3D(0., 0., (aerogelThickness + airgapThickness)/2.);
    auto airgapPV = gasvolVol.placeVolume(airgapVol, airgapPlacement);
    DetElement airgapDE(det, "airgap_de", 2);
    airgapDE.setPlacement(airgapPV);

    // Filter placement (after airgap)
 auto filterPlacement = Translation3D(0., 0., airgapThickness) *  // <-- This extra translation is critical
                     Translation3D(radiatorPos) * 
                     RotationY(radiatorPitch) * 
                     Translation3D(0., 0., (aerogelThickness + filterThickness)/2.);
    auto filterPV = gasvolVol.placeVolume(filterVol, filterPlacement);
    DetElement filterDE(det, "filter_de", 3);
